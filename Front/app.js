         ...
         function _refreshPageData() {
             $http({
                 ...
                 for (var i = 0; i < response.data.length; i++) {
                     if (response.data[i].status != 1 && response.data[i].utilisateur.id == $scope.currentUser.id) {
                         $scope.articles[j++] = response.data[i];
                     }
                 }
             }, function errorCallback(response) {
                 console.log(response.statusText);
     ...
     function($scope, $http, $location) {
         ...
         $scope.connexion = function() {
             $http({
                 ...
                 response.data.lastConnected = new Date();
     ...
     function($scope, $http, $location) {
         ...
         $scope.submitUser = function() {
             ...
             if ($scope.form.id == -1) {
                 ...
                 method = "POST";
                 url = 'http://localhost:8080/WS_Blog/webresources/entity.utilisateurs';
                 currentUser.lastConnected = new Date();
